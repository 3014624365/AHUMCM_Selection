...
output_dir = "camera_simulation_results"
...
def simulate_camera_perspective(img, theta_deg=30, phi_deg=20, scale=0.8):
    """
    Simulate a camera perspective transformation on the image

    Parameters:
        img: Original image
        theta_deg: Rotation angle around x-axis (degrees)
        phi_deg: Rotation angle around y-axis (degrees)
        scale: Scale factor for the projection

    Returns:
        transformed_img: Image with simulated camera perspective
    ...
    # Project to 3D and apply rotation
    src_3d = np.zeros((4, 3))
    src_3d[:, :2] = src_pts
    src_3d[:, 2] = 0  # z = 0 for all points (planar)

    # Center the points at origin before rotation
    src_3d[:, 0] -= w / 2
    src_3d[:, 1] -= h / 2
...
def inverse_perspective_transform(img, M):
    """
    Apply inverse perspective transformation to recover original view

    Parameters:
        img: Transformed image
        M: Transformation matrix from the original transformation

    Returns:
        recovered_img: Image with inverse transformation applied
    """
    h, w = img.shape[:2]
...
if __name__ == "__main__":
    ...
    if original_img is None:
        print(f"Error: Unable to read the original image '{original_image_path}'")
        exit(1)

    print(f"Successfully read original image: {original_image_path}")
    print(f"Image size: {original_img.shape[1]} x {original_img.shape[0]} pixels")

    # Step 1: Run original analysis to get ground truth
    print("\n" + "=" * 60)
    print("Step 1: Analyzing original image for ground truth...")
    print("=" * 60)

    ground_truth_sag = find_max_sag_with_advanced_catenary(
        original_image_path,
        exclude_margin=15,
        output_dir=os.path.join(output_dir, "original_analysis")
    )

    # Step 2: Simulate camera perspective
    print("\n" + "=" * 60)
    print("Step 2: Simulating camera perspective...")
    print("=" * 60)

    # Define camera parameters
    theta = 20  # Rotation around x-axis (degrees)
    phi = 15  # Rotation around y-axis (degrees)
    scale = 0.9  # Scale factor

    # Apply camera perspective transformation
    camera_view_img, transformation_matrix = simulate_camera_perspective(
        original_img, theta_deg=theta, phi_deg=phi, scale=scale
    )

    # Save the simulated camera view
    camera_view_path = os.path.join(output_dir, "camera_view.png")
    cv2.imwrite(camera_view_path, camera_view_img)
    print(f"Simulated camera view saved to: {camera_view_path}")
    print(f"Camera parameters: theta={theta}°, phi={phi}°, scale={scale}")

    # Step 3: Apply inverse transformation
    print("\n" + "=" * 60)
    print("Step 3: Applying inverse transformation...")
    print("=" * 60)

    # Recover the original perspective
    recovered_img = inverse_perspective_transform(camera_view_img, transformation_matrix)

    # Save the recovered image
    recovered_path = os.path.join(output_dir, "recovered_image.png")
    cv2.imwrite(recovered_path, recovered_img)
    print(f"Recovered image saved to: {recovered_path}")

    # Step 4: Analyze the recovered image
    print("\n" + "=" * 60)
    print("Step 4: Analyzing recovered image...")
    print("=" * 60)

    recovered_sag = find_max_sag_with_advanced_catenary(
        recovered_path,
        exclude_margin=15,
        output_dir=os.path.join(output_dir, "recovered_analysis")
    )
    ...
    if ground_truth_sag is not None and recovered_sag is not None:
        error = abs(ground_truth_sag - recovered_sag)
        error_percent = (error / ground_truth_sag) * 100

        print(f"Ground truth maximum sag: {ground_truth_sag:.2f} meters")
        print(f"Recovered maximum sag:   {recovered_sag:.2f} meters")
        print(f"Absolute error:          {error:.2f} meters")
        ...
        plt.subplot(2, 2, 2)
        plt.imshow(camera_view_img, cmap='gray')
        plt.title(f'Simulated Camera View\n(θ={theta}°, φ={phi}°)')
        plt.axis('off')
        ...
        # Comparison of sag values
        plt.subplot(2, 2, 4)
        bar_positions = [1, 2]
        sag_values = [ground_truth_sag, recovered_sag]
        bars = plt.bar(bar_positions, sag_values, color=[colors[0], colors[2]])

        # Add value labels on bars
        for bar, value in zip(bars, sag_values):
            plt.text(bar.get_x() + bar.get_width() / 2, value + 0.1,
                     f'{value:.2f}m', ha='center', va='bottom')

        plt.xticks(bar_positions, ['Ground Truth', 'Recovered'])
        plt.ylabel('Maximum Sag (meters)')
        plt.title(f'Sag Comparison\nError: {error:.2f}m ({error_percent:.2f}%)')
        plt.grid(True, axis='y')

        plt.tight_layout()
        comparison_path = os.path.join(output_dir, "comparison_summary.png")
        plt.savefig(comparison_path, dpi=300, bbox_inches='tight')
        print(f"Comparison summary saved to: {comparison_path}")
        plt.close()

    else:
        print("Couldn't compare results because at least one analysis failed.")

    print("\n" + "=" * 60)
    print(f"Camera simulation complete! Results saved to '{output_dir}'")
    print("=" * 60)
